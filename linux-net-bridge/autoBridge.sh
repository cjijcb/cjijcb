...
============================================="
echo "[1/9] detecting existing bridge interface"
...
do
  ...
  if [ "$SLAVE_TYPE" = "bridge" ]; then
    ...
    SLAVE_ETHERTNET_DEV_NAME=$( nmcli con show "$PORT_BRIDGE_CON_NAME" | sed -En "s/connection\.interface-name:[[:space:]]+([^[:space:]].*)/\1/p" )
    MASTER_BRIDGE_DEV_NAME=$( nmcli con show "$PORT_BRIDGE_CON_NAME" | sed -En "s/connection\.master:[[:space:]]+([^[:space:]].*)/\1/p" )
    MASTER_BRIDGE_CON_NAME=$( nmcli dev show "$MASTER_BRIDGE_DEV_NAME" | sed -En "s/GENERAL\.CONNECTION:[[:space:]]+([^[:space:]]+.*)/\1/p" )
  break
  fi
done
#
...
echo "[7/9] creating bridge interface..."
nmcli con add type bridge ifname bridge0 con-name bridge0 > /dev/null && \
echo "[8/9] configuring bridge interface..."
...
echo "[9/9] enabling bridge interface..."
